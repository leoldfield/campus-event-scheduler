...
export default function Events() {
  ...
  const handleRegister = async (eventId) => {
    ...
    try {
      ...
      console.error(err);
      // If EventContext already handled ALREADY_EXISTS silently,
      // this catch only fires for genuine failures.
      const alreadyExists =
        err?.message?.includes("registration_pkey") ||
        err?.message?.includes("ALREADY_EXISTS") ||
        JSON.stringify(err)?.includes("ALREADY_EXISTS");
      addNotification({
        type: alreadyExists ? "info" : "error",
        title: alreadyExists ? "Already registered" : "Error",
